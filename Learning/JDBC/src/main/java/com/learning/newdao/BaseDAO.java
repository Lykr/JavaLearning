...
public abstract class BaseDAO<T> {
    ...
    {
        //获取当前 BaseDAO 的子类继承的父类中的范型
        Type genericSuperclass = this.getClass().getGenericSuperclass();
        ParameterizedType parameterizedType = (ParameterizedType) genericSuperclass;

        Type[] typeArguments = parameterizedType.getActualTypeArguments(); //获取了父类的范型
        clazz = (Class<T>) typeArguments[0]; //范型的第一个参数
    }
    ...
    //通用单对象查询
    public T getInstance(Connection connection, String sql, Object... objs) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
    ...
    //通用多对象查询
    public List<T> getInstanceList(Connection connection, String sql, Object... objs) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ps = connection.prepareStatement(sql); //准备 PreparedStatement
